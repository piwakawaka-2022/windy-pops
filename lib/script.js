...
function toggleClassCompliments() { //add the toggle between angry and happy container
...
function placeCompliment() {
  ...
  newText.textContent= "compliment :-)" //This will be the variable returned by the return compliment func
...
function placeSpicy() {
  ...
  newText.textContent= "Insult >:(" //This will be the variable returned by the return spicy func
  let xPos = randomPositionGenerator(width)
  let yPos = randomPositionGenerator(height)
  let rotation = randomRotation();
  newText.style.setProperty('position', 'absolute')
